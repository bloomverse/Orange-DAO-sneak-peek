...
namespace TPSBR.UI
{
	public class UIHopscotchGameplayView : UIView
	{
		// PRIVATE MEMBERS
		...
		[SerializeField]
		private GameObject miniScoreBoard;
		[SerializeField]
		...
		[SerializeField]
		private AudioSetup _interactionInfoSound;
		...
		protected override void OnInitialize()
		{
			...
			Debug.Log(Context.GameplayMode + "------------- gameplaymode" + (Context.GameplayMode is not LobbyTutorialGameplayMode));
			// Hide panel for game mode specific UI

			_battleRoyale.SetActive(Context.GameplayMode is BattleRoyaleGameplayMode);

			if (Context.GameplayMode is LobbyGameplayMode || Context.GameplayMode is TutorialGameplayMode || Context.GameplayMode is LobbyTutorialGameplayMode)
			{
				playerPanel.SetActive(false);
				miniScoreBoard.SetActive(false);
			}

			//_musicVolumeSlider.SetActive(Context.GameplayMode is TutorialGameplayMode);
