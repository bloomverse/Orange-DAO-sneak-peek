...
namespace TPSBR
{
	public class PlayerService : IGlobalService
	{
		...
		private PlayerData LoadPlayer()
		{
			...
			Debug.Log("Player DAta NULO!!!!!"+ playerData==null);
			...
			if (playerData == null)
			{
				...
				playerData.AgentID = Global.Settings.Agent.GetRandomAgentSetup().ID;
				 if(PersistentStorage.GetString("Skin")!=null  ){
				playerData.SelectedSkin = PersistentStorage.GetString("Skin");
			}  else{
				playerData.SelectedSkin = "defaultSkin";
			}

				
			};

		//	var playerData = new PlayerData(userID);
			//	playerData.AgentID = Global.Settings.Agent.GetRandomAgentSetup().ID;
			return playerData;
		}
