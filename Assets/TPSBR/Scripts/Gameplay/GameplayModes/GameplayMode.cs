...
namespace TPSBR
{
	...
	public abstract class GameplayMode : ContextBehaviour
	{
		...
		public void Activate(bool middleEvents = false, bool tournamentMode = false)
		{

			_middleEvents = middleEvents;
		...
		public void AgentDeath(Agent victim, HitData hitData)
		{
			...
			//MECAN
			 if (victim.GetComponentInChildren<NetworkMecanimAnimator>() != null)
			{
				float FullBodyLayerWeight = Mathf.Lerp(0, 1f, 6f * Time);
				victim.GetComponentInChildren<NetworkMecanimAnimator>().Animator.SetLayerWeight(victim.GetComponentInChildren<NetworkMecanimAnimator>().Animator.GetLayerIndex("FullBody"), FullBodyLayerWeight);
				victim.GetComponentInChildren<NetworkMecanimAnimator>().Animator.SetBool("isDead", true);
			}
		...
		protected Agent SpawnAgent(PlayerRef playerRef, Vector3 position, Quaternion rotation)
		{
			...
			(Runner, NO) => NO.GetComponent<Agent>().setOptions(weaponsAllowed,player.Nickname, player.SelectedSkin, player.AgentID));
