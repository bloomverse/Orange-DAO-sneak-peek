namespace TPSBR
{
	...
	public sealed class CharacterAnimationController : AnimationController
	{
		...
		public bool CanJump()
		{
			...
			this.GetComponent<Agent>().jumpCount = 0;
			return true;
		}
		...
		private void SnapWeapon()
		{
			...
			if (HasInputAuthority == true || _agent.IsObserved == true)
			{
				...
				spine.transform.rotation = Quaternion.Euler(new Vector3(snapRotation.x, snapRotation.y + angle, snapRotation.z));
				//weaponHandle.rotation = Quaternion.Euler(snapRotation);
			}
			else
			{
				spine.transform.localRotation = Quaternion.LookRotation(_kcc.FixedData.LookDirection);
